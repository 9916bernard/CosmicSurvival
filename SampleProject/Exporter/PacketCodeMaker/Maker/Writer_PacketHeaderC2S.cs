...
{
    public class Writer_PacketHeaderC2S
    {
        static public void Write(Util.PathInfo pathInfo, PacketInfos infos, bool writeBot, bool writeClient)
        {
            ...
            string filepath_server = string.Format("{0}NetPacket_{1}_C2S.h", server_directory, infos.mServerName);

            Write_C2S(filepath_server, infos, true);
            ...
            if (writeBot == true)
            {
                ...
                string filepath_bot = string.Format("{0}NetPacket_{1}_C2S.h", bot_directory, infos.mServerName);

                Write_C2S(filepath_bot, infos, false);
            ...
            if (writeClient == true)
            {
                ...
                string filepath_client = string.Format("{0}NetPacket_{1}_C2S.h", client_directory, infos.mServerName);

                Write_C2S(filepath_client, infos, false);
                Console.WriteLine("=== Write {0}", filepath_client);
            }
        }

        static private void Write_C2S(string filepath, PacketInfos infos, bool isServer)
        {
        ...
        {
            w.WriteLine("struct NP{0}C2S_{1} : public NP{2}C2SHeader", infos.mServerName, p.cmd_name, infos.mServerName);
            w.WriteLine("{");
            ...
            if (p.elems.Count > 0)
            {
                w.WriteLine("{0}NP{1}C2S_{2}() : NP{3}C2SHeader(sizeof(*this), NP_{4}_C2S::{5}) {{}}", Util.Tab, infos.mServerName, p.cmd_name, infos.mServerName, infos.mServerNameUP, p.cmd_name.ToUpper());
            }

            w.Write("{0}NP{1}C2S_{2}(", Util.Tab, infos.mServerName, p.cmd_name);
            for (int i = 0; i < p.elems.Count; ++i)
            {
                PacketInfos.Element elem = p.elems[i];
                if (elem.vtype == "string")
            ...
            w.WriteLine("{0}{1}: NP{2}C2SHeader(sizeof(*this), NP_{3}_C2S::{4})", Util.Tab, Util.Tab, infos.mServerName, infos.mServerNameUP, p.cmd_name.ToUpper());
            ...
            for (int i = 0; i < p.elems.Count; ++i)
            {
                ...
                if (elem.vtype == "string")
                {
                    ...
                    else
                    {
                        ...
                        w.WriteLine("{0}{1}m{2}[{3}.GetCharArray().Num() >= {4} ? {5} : {6}.GetCharArray().Num()] = 0;", Util.Tab, Util.Tab, elem.vname, elem.vname.ToLower(), elem.vcount + 1, elem.vcount, elem.vname.ToLower());
                    }
                }
