...
{
    public class Writer_S2C_Receiver
    {
        static public void Write(Util.PathInfo pathInfo, PacketInfos infos)
        {
            string filepath_client_header = string.Format("{0}Net_{1}_S2C.h", pathInfo.mCodePathClient, infos.mServerName);
            string filepath_bot_header = string.Format("{0}Net_{1}_S2C.h", pathInfo.mCodePathBot, infos.mServerName);
