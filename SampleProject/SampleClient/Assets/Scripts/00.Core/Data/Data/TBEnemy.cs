...
public class UTBEnemy_Record
{
    ...
    {
        EnemyID = fs.ReadInt32();
        Name = fs.ReadInt32();
        ushort cnt = 0;
        cnt = fs.ReadUInt16();
        if (cnt > 0) { fs.Read(buffer, 0, cnt * 2); Info = System.Text.Encoding.Unicode.GetString(buffer, 0, cnt * 2); }
        cnt = fs.ReadUInt16();
        if (cnt > 0) { fs.Read(buffer, 0, cnt * 2); Grade = System.Text.Encoding.Unicode.GetString(buffer, 0, cnt * 2); }
        Health = fs.ReadInt32();
        Speed = fs.ReadSingle();
        UpgradeInterval = fs.ReadSingle();
        UpgradeHealth = fs.ReadInt32();
        UpgradeSpeed = fs.ReadSingle();
        SpawnTime = fs.ReadInt32();
        SpawnChance = fs.ReadInt32();
        SpawnInterval = fs.ReadInt32();
    }
};

public class UTBEnemy
{
    public Dictionary<int, UTBEnemy_Record> mapTable = new Dictionary<int, UTBEnemy_Record>();
    ...
    public bool LoadFromStream(BinaryReader fs, byte[] buffer)
    {
        ...
        {
            UTBEnemy_Record rec = new UTBEnemy_Record();

            rec.Read(fs, buffer);

            mapTable.Add(rec.EnemyID, rec);
        }
    ...
    {
        TextAsset textAsset = Resources.Load("Table/Enemy") as TextAsset;
    ...
    {
        string filePath = Application.temporaryCachePath + "/Table/Enemy.txt";
    ...
    public UTBEnemy_Record Find(int key1)
    {
        UTBEnemy_Record rec = null;
