...
public class UTBFund_Record
{
    ...
    {
        byte val_FundType = fs.ReadByte(); FundType = (ETB_FUND)val_FundType;
        ushort cnt = 0;
        cnt = fs.ReadUInt16();
...
public class UTBFund
{
    public Dictionary<ETB_FUND, UTBFund_Record> mapTable = new Dictionary<ETB_FUND, UTBFund_Record>();
    ...
    public bool LoadFromStream(BinaryReader fs, byte[] buffer)
    {
        ...
        {
            UTBFund_Record rec = new UTBFund_Record();

            rec.Read(fs, buffer);

            mapTable.Add(rec.FundType, rec);
        }
    ...
    {
        TextAsset textAsset = Resources.Load("Table/Fund") as TextAsset;
    ...
    {
        string filePath = Application.temporaryCachePath + "/Table/Fund.txt";
    ...
    public UTBFund_Record Find(ETB_FUND key1)
    {
        UTBFund_Record rec = null;
