...
public class UIUpgradeUnitItem : MonoBehaviour
{
    ...
    public void Set(int InIndexInList, FTB_UpgradeMineral InData)
    {
        ...
        _UpRec = TABLE.upgrade.Find(1000 + InIndexInList);

        if (_Rec == null)
        {
            Debug.Log($"===== UIUpgradeUnitItem _Rec is null : {InIndexInList} => {InData.FundType}");
            return;
        }

        if (_Rec == null)
        {
            Debug.Log($"===== UIUpgradeUnitItem _UpRec is null : {InIndexInList} => {1000 + InIndexInList}");
            return;
        }
        ...
        _Text_Upgrade_Name.SetText(41003 + InIndexInList);
        ...
        var upEffLv = USER.upgrade.GetUpgradeEffectLevel(_UpRec.UpgradeType);

        Debug.Log($"===== UIUpgradeUnitItem : {InIndexInList} => {upEffLv} => {_UpRec.UpgradeCost}");
    }
